...
- [x] Sistema de misiones (`inc/misiones.php`): una microclase puede llevar, como mucho, una misión — "casa" (solo marcar hecha) o "taller" (con foto de evidencia). **Decisión de producto confirmada por el usuario**: cuando la clase lleva misión, resolverla es lo que la completa; ver el video ya no alcanza. Elegir "lo haré después" deja la clase pendiente sin desbloquear la siguiente. Recompensa: 10 monedas la de casa, 20 la de taller — valores de partida, no un balance de juego definido por el cliente.
- [x] Monedas y XP: se ganan resolviendo misiones y van **a la cuenta del padre, no al hijo** (el concepto dice explícitamente que la experiencia es "del padre" — si tiene varios hijos, todos suman a la misma mochila). Se ven en `/panel` junto al saludo.
- [ ] Estrellas, insignias visuales y certificados — todavía no existen. Las "habilidades" ya se muestran al terminar un curso (ver `inc/progreso.php`), pero sin insignia gráfica ni certificado descargable.
...
- [ ] Límite de intentos de inicio de sesión: ni el formulario ni el endpoint AJAX cuentan los intentos fallidos. No es una puerta nueva (el formulario ya estaba igual de expuesto, y `wp-login.php` también), pero conviene resolverlo antes de abrir el sitio — con un plugin de seguridad o con un contador propio por IP y por correo.
...
- [x] Panel por rol: quien puede editar cursos (administrador e instructor) ve en `/panel` su propio panel de contenido — cifras del catálogo, perfiles por etapa, avance agregado por curso y enlaces al escritorio — en vez del panel de familia. Con `?vista=familia` puede ver la plataforma como la ve un padre, para probarla.
- [ ] Decidir con el cliente si el equipo debe poder ver familias **una por una** (nombre del hijo, su avance). Hoy solo se muestran cifras agregadas, a propósito: son datos de menores y para decidir qué contenido producir alcanza con el agregado. Si lo piden, conviene dejar por escrito quién puede verlo y para qué.
- [x] Administración desde el panel: crear, editar y eliminar cursos y microclases, y ver y administrar usuarios, sin entrar al escritorio. A pedido expreso del usuario, que prefiere que su equipo trabaje sin salir del panel.
- [x] Editor con formato (negrita, cursiva, listas, enlaces) en los campos de descripción del panel, e imagen destacada en cursos y microclases.
- [ ] Decidir el proveedor de video con el cliente. **YouTube no permite quitar botones sueltos** del reproductor: o se muestran todos los controles o ninguno. Se le quitó todo lo que sí se puede (videos sugeridos de otros canales, anotaciones, cookies de seguimiento antes del play), pero siguen a la vista ajustes, subtítulos y compartir. Si el reproductor limpio importa, **Vimeo** permite mucho más control, y un video subido al propio sitio permite quitar descarga, velocidad y miniatura flotante.
- [ ] Suscripciones en el panel del equipo: no existen todavía (falta definir el modelo de cobro, ver bloque 0).
- [x] Guía de primeros pasos para el padre recién registrado: cuatro pasos que se marcan solos según lo que de verdad hizo, con el botón de "qué toca ahora" en el paso pendiente. Desaparece sola al completarlos, o a mano.
- [x] El escritorio de WordPress y su barra gris quedan fuera del alcance de las cuentas de padre.
- [ ] Revisar si conviene esconderle también el escritorio al instructor, ahora que puede trabajar entero desde el panel. Ojo: sigue necesitando el escritorio para restaurar de la papelera y para la ficha de un usuario.
